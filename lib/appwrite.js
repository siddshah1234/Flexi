...
export async function signInWithGoogle() {
  ...
  // Create a user document if this is a first-time Google sign-in
  ...
  if (!userDoc) {
    ...
    userDoc = await databases.createDocument(
      ...
      ID.unique(),
      {
        accountId: acct.$id,
        email: acct.email,
        username: acct.name || acct.email.split('@')[0],
        avatar: `https://ui-avatars.com/api/?name=${encodeURIComponent(acct.name || acct.email)}`,
        xp: 0,
      }
    );
  }
